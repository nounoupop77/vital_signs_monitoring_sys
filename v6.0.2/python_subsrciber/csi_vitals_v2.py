...
# paho-mqtt CallbackAPIVersion.VERSION2 signatures:
...
def on_connect(client, userdata, connect_flags, reason_code, properties=None):
    if reason_code == 0:
        ...
        print(f"[{now()}] CONNECT FAILED rc={reason_code}")


def on_disconnect(client, userdata, disconnect_flags, reason_code, properties=None):
    print(f"[{now()}] DISCONNECTED rc={reason_code} (auto-reconnecting)")
...
def main():
    ...
    # Timestamped output files: each session gets its own file, so an
    # accidental editor save can never wipe a previous recording, and
    # sessions never append into one another's data.
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    RAW_PATH = os.path.join(base, f"csi_raw_v2_{stamp}.ndjson")
    CSV_PATH = os.path.join(base, f"csi_results_v2_{stamp}.csv")
